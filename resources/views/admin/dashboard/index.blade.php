...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <!-- Total Films -->
        <div class="p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800 flex items-center justify-between shadow-xl">
            <div>
                ...
                <p class="text-[11px] text-zinc-400 mt-0.5">{{ number_format($stats['total_movies']) }} Movie • {{ number_format($stats['total_series']) }} Series • {{ number_format($stats['total_dracin'] ?? 0) }} Dracin</p>
            </div>
        ...
        <div class="p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800 flex items-center justify-between shadow-xl">
            ...
            <div class="w-11 h-11 rounded-2xl bg-sky-500/10 border border-sky-500/20 text-sky-400 flex items-center justify-center shrink-0">
        ...
        <div class="p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800 flex items-center justify-between shadow-xl">
            ...
            <div class="w-11 h-11 rounded-2xl bg-purple-500/10 border border-purple-500/20 text-purple-400 flex items-center justify-center shrink-0">
        ...
        <div class="p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800 flex items-center justify-between shadow-xl">
            <div>
                ...
                <p class="text-[11px] text-zinc-400 mt-0.5">{{ number_format($stats['total_genres']) }} Genre Terdaftar</p>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800 space-y-4 shadow-xl">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <div class="w-full h-2 rounded-full bg-zinc-950 overflow-hidden">
                                <div class="h-full bg-sky-500 rounded-full" style="width: {{ $moviePct }}%"></div>
                            </div>
                        </div>
                ...
                <div class="p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800 space-y-4 shadow-xl">
                    ...
                    <div class="flex flex-wrap gap-2 text-xs">
                        @foreach(['SU', 'G', 'PG', '13+', '16+', '18+'] as $rating)
                            ...
                            <div class="px-3 py-1.5 rounded-xl bg-zinc-950 border border-zinc-800 flex items-center gap-2">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.films.index') }}" class="text-xs text-amber-400 hover:underline">Kelola Katalog &rarr;</a>
                </div>
                            ...
                            @forelse($mostViewedFilms as $film)
                                <tr class="hover:bg-zinc-800/40 transition-colors group">
                                    <td class="px-4 py-3 flex items-center gap-3">
                                        <img src="{{ $film->poster_url }}" class="w-8 h-12 object-cover rounded-lg shrink-0 bg-zinc-950">
                                        <div>
                                    ...
                                    <td class="px-4 py-3">
                                        <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase {{ $film->subject_type === 'dracin' ? 'bg-rose-500/10 text-rose-300 border border-rose-500/30' : ($film->subject_type === 'series' ? 'bg-purple-500/10 text-purple-300 border border-purple-500/30' : 'bg-sky-500/10 text-sky-300 border border-sky-500/30') }}">
                                            {{ $film->subject_type }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-right font-mono font-bold text-zinc-200 text-xs">
                                        {{ number_format($film->view_count) }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-8 text-center text-zinc-500">Belum ada data ditonton.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="space-y-8">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.activity_logs.index') }}" class="text-xs text-sky-400 hover:underline">Semua Log &rarr;</a>
                </div>

                <div class="p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800 space-y-3 shadow-xl">
                    @forelse($recentActivityLogs as $log)
                        <div class="p-3 rounded-xl bg-zinc-950 border border-zinc-800 text-xs space-y-1">
                            <div class="flex items-center justify-between">
                                <span class="font-bold text-amber-400 uppercase text-[10px] font-mono">{{ $log->action }}</span>
            ...
            <div class="space-y-4">
                ...
                <div class="p-5 rounded-2xl bg-zinc-900/90 border border-zinc-800 space-y-4 shadow-xl">
                    <div>
                        ...
                        <p class="text-xs font-bold text-amber-400 mt-0.5 font-mono">{{ $lastSyncAt }}</p>
                    </div>

                    <div class="p-3.5 rounded-xl bg-zinc-950 border border-zinc-800 text-xs text-zinc-300 font-mono">
                        {{ $lastSyncStatus }}
                    </div>
