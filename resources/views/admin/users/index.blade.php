...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-3 text-xs">
        <!-- Total Users Shortcut -->
        <a href="{{ route('admin.users.index') }}" 
           class="p-3 sm:p-4 rounded-2xl bg-zinc-900/90 border transition-all flex flex-col justify-between hover:scale-[1.01] shadow-md group {{ !request()->hasAny(['status', 'search']) ? 'border-amber-500/40 bg-amber-500/5' : 'border-zinc-800 hover:border-zinc-700' }}">
            <div class="flex items-center justify-between">
                <span class="text-zinc-400 font-semibold group-hover:text-white transition-colors text-[11px] sm:text-xs truncate">Total Pengguna</span>
                <i data-lucide="users" class="w-4 h-4 text-zinc-500 group-hover:text-amber-400 transition-colors shrink-0"></i>
            </div>
            <span class="font-extrabold text-white text-lg sm:text-xl mt-2 font-['Outfit']">{{ number_format($stats['total'] ?? $users->total()) }}</span>
        </a>

        <!-- Active Users Shortcut -->
        <a href="{{ route('admin.users.index', ['status' => 'active']) }}" 
           class="p-3 sm:p-4 rounded-2xl bg-zinc-900/90 border transition-all flex flex-col justify-between hover:scale-[1.01] shadow-md group {{ request('status') === 'active' ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-zinc-800 hover:border-emerald-500/30' }}">
            <div class="flex items-center justify-between">
                <span class="text-zinc-400 font-semibold group-hover:text-emerald-300 transition-colors text-[11px] sm:text-xs truncate">Pengguna Aktif</span>
                <i data-lucide="user-check" class="w-4 h-4 text-emerald-400 shrink-0"></i>
            </div>
            <span class="font-extrabold text-emerald-400 text-lg sm:text-xl mt-2 font-['Outfit']">{{ number_format($stats['active'] ?? 0) }}</span>
        </a>

        <!-- Administrator (Superadmin) Shortcut -->
        <a href="{{ route('admin.users.index', ['status' => 'administrator']) }}" 
           class="p-3 sm:p-4 rounded-2xl bg-zinc-900/90 border transition-all flex flex-col justify-between hover:scale-[1.01] shadow-md group {{ request('status') === 'administrator' ? 'border-amber-500/50 bg-amber-500/15' : 'border-zinc-800 hover:border-amber-500/30' }}">
            <div class="flex items-center justify-between">
                <span class="text-zinc-400 font-semibold group-hover:text-amber-300 transition-colors text-[11px] sm:text-xs truncate">Administrator</span>
                <i data-lucide="crown" class="w-4 h-4 text-amber-400 shrink-0"></i>
            </div>
            <span class="font-extrabold text-amber-400 text-lg sm:text-xl mt-2 font-['Outfit']">{{ number_format($stats['administrator'] ?? 0) }}</span>
        </a>

        <!-- Admin Konten Shortcut -->
        <a href="{{ route('admin.users.index', ['status' => 'admin']) }}" 
           class="p-3 sm:p-4 rounded-2xl bg-zinc-900/90 border transition-all flex flex-col justify-between hover:scale-[1.01] shadow-md group {{ request('status') === 'admin' ? 'border-sky-500/50 bg-sky-500/15' : 'border-zinc-800 hover:border-sky-500/30' }}">
            <div class="flex items-center justify-between">
                <span class="text-zinc-400 font-semibold group-hover:text-sky-300 transition-colors text-[11px] sm:text-xs truncate">Admin Konten</span>
                <i data-lucide="shield" class="w-4 h-4 text-sky-400 shrink-0"></i>
            </div>
            <span class="font-extrabold text-sky-400 text-lg sm:text-xl mt-2 font-['Outfit']">{{ number_format($stats['admin'] ?? 0) }}</span>
        </a>

        <!-- Ad-Free Users Shortcut -->
        <a href="{{ route('admin.users.index', ['status' => 'ad_free']) }}" 
           class="p-3 sm:p-4 rounded-2xl bg-zinc-900/90 border transition-all flex flex-col justify-between hover:scale-[1.01] shadow-md group {{ request('status') === 'ad_free' ? 'border-amber-400/50 bg-amber-400/10' : 'border-zinc-800 hover:border-amber-400/30' }}">
            <div class="flex items-center justify-between">
                <span class="text-zinc-400 font-semibold group-hover:text-amber-300 transition-colors text-[11px] sm:text-xs truncate">Bebas Iklan</span>
                <i data-lucide="sparkles" class="w-4 h-4 text-amber-400 shrink-0"></i>
            </div>
            <span class="font-extrabold text-amber-300 text-lg sm:text-xl mt-2 font-['Outfit']">{{ number_format($stats['ad_free'] ?? 0) }}</span>
        </a>

        <!-- Banned Users Shortcut -->
        <a href="{{ route('admin.users.index', ['status' => 'banned']) }}" 
           class="p-3 sm:p-4 rounded-2xl bg-zinc-900/90 border transition-all flex flex-col justify-between hover:scale-[1.01] shadow-md group {{ request('status') === 'banned' ? 'border-rose-500/40 bg-rose-500/10' : 'border-zinc-800 hover:border-rose-500/30' }}">
            <div class="flex items-center justify-between">
                <span class="text-zinc-400 font-semibold group-hover:text-rose-300 transition-colors text-[11px] sm:text-xs truncate">Banned</span>
                <i data-lucide="user-x" class="w-4 h-4 text-rose-400 shrink-0"></i>
            </div>
            <span class="font-extrabold text-rose-400 text-lg sm:text-xl mt-2 font-['Outfit']">{{ number_format($stats['banned'] ?? 0) }}</span>
        </a>
    </div>
