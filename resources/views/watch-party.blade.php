                ...
                <template x-if="activeStream">
                    <div class="relative w-full h-full flex items-center justify-center bg-black">
                        ...
                        <div class="absolute inset-0 z-25 pointer-events-none flex items-center justify-center gap-6 sm:gap-14 transition-opacity duration-300"
                            ...
                            <button @click.stop="if(isHost) { seek(currentTime - 5); triggerRipple('rewind'); }" 
                                    :disabled="!isHost"
                            ...
                            <button @click.stop="if(isHost) { seek(currentTime + 5); triggerRipple('forward'); }" 
                                    :disabled="!isHost"
                                    :class="isHost ? 'cursor-pointer hover:scale-110 active:scale-95' : 'cursor-not-allowed opacity-50'"
                                    class="w-11 h-11 sm:w-14 sm:h-14 rounded-full glass-panel flex flex-col items-center justify-center border border-white/20 text-white shadow-2xl transition-all bg-black/40 backdrop-blur-md"
                                    title="+5 Detik (Host Only)">
                    ...
                    <div class="absolute inset-x-0 top-0 z-40 p-2.5 sm:p-5 transition-opacity duration-300 flex items-center justify-between gap-2 pointer-events-auto"
                        ...
                        <div class="flex items-center gap-1 sm:gap-2 shrink-0">
                            ...
                            <div class="relative" @click.outside="speedDropdownOpen = false">
                                <button @click.stop="if(isHost) { speedDropdownOpen = !speedDropdownOpen; audioDropdownOpen = false; qualityDropdownOpen = false; subtitleDropdownOpen = false; aspectRatioDropdownOpen = false; }" 
                        ...
                        <div class="flex items-center justify-between gap-1 sm:gap-4 py-0.5 relative z-40">
                            ...
                            <div class="flex items-center gap-0.5 sm:gap-2.5 shrink-0">
                                ...
                                <button @click.stop="if(isHost) { seek(currentTime - 5); triggerRipple('rewind'); }" 
                                        ...
                                        class="p-1.5 sm:p-2 rounded-xl transition-colors hidden sm:flex items-center justify-center" title="-5 Detik (Host Only)">
        ...
        return {
            ...
            async setAudioTrack(track) {
                ...
                if (typeof track === 'object' && track.subjectId) {
                    ...
                    try {
                        const res = await fetch(`/moviebox/resources/${track.subjectId}?se=${this.currentSeason}&ep=${this.currentEpisode}`);
                        if (res.ok) {
                            const data = await res.json();
                            const rawList = (data.list || []).sort((a, b) => {
                            ...
                            for (const r of rawList) {
                                ...
                                parsedQualities.push({
                                    ...
                                    url: '/moviebox/proxy-stream?url=' + encodeURIComponent(rawUrl) + '&id=' + track.subjectId
                                });
                            }
                            ...
                            if (this.qualities.length > 0) {
                                this.activeStream = this.qualities[0].url;
                            }
                            ...
                            this.$nextTick(() => {
                                if (this.$refs.video) {
                                    ...
                                    if (wasPlaying) this.$refs.video.play().catch(() => {});
                                }
                                if (window.lucide) lucide.createIcons();
                            });
                        }
